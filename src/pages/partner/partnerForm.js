...
import { createPartner, fetchPartnerById, updatePartner } from '../../api/partner/partnerApi';
...
export default function PartnerForm() {
        ...
        if (isEdit) {
            ...
            (async () => {
                try {
                    ...
                    setErrorModal({ open: true, message: error.response?.data?.error || 'Failed to fetch partner details for editing.' });
                } finally {
                    setLoading(false);
                }
            })();
    ...
    const handleSubmit = async e => {
        ...
        try {
            if (isEdit) {
                await updatePartner(id, form, token);
            } else {
                await createPartner(form, token);
            }
            nav('/admin/partner/partner');
        } catch (error) {
            console.error("Error submitting partner:", error);
            if (error.response && error.response.status === 400) {
                setErrorModal({ open: true, message: error.response.data.error });
            } else {
                setErrorModal({ open: true, message: error.message || (isEdit ? 'Failed to update partner.' : 'Failed to create partner.') });
            }
        } finally {
            setLoading(false);
        }
